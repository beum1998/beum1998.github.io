...
export type ToolCommand =
  | { action: "getState" }
  | { action: "saveTimeLeft"; timeKey?: string | null }
  | { action: "setTime"; timeKey: string; panel?: "before" | "after" | "both" }
  | { action: "setFilters"; categories: string[] }
  | { action: "clearFilters" }
  | { action: "setCameraSync"; enabled: boolean }
  | { action: "readTimeLog"; timeKey?: string }
  | { action: "readQuantityLog"; timeKey?: string }
  | { action: "readShapeLog"; elementIds: string[] }
  | { action: "getVisibleElements"; panel?: "before" | "after" }
  | { action: "getVisibleByCategory"; panel?: "before" | "after"; category: string }
  | { action: "getVisibleStats"; panel?: "before" | "after" }
  | { action: "compareVisibleStats" }
  | { action: "colorElements"; elementIds: string[]; color?: string } // by ids
  | { action: "setColor"; category: string; color: string } // by category
  | { action: "resetColors" }
  | { action: "restoreColors" }
  | { action: "highlightChanged"; color?: string }
  | { action: "setChangeFilter"; changeType: "added" | "deleted" | "modified" }
  | { action: "isolate"; categories: string[] }
  | { action: "clearIsolation" };

export type ToolResult = Record<string, unknown>;

function safeParseJsonArray(jsonText: string): unknown[] | null {
  try {
    const parsed = JSON.parse(jsonText);
    if (Array.isArray(parsed)) return parsed;
    // allow single object fallback, but normalize into array
...
export function extractCommands(text: string): ToolCommand[] {
  ...
  for (const block of blocks) {
    ...
    for (const item of list) {
      const cmd = item as ToolCommand;
      if (cmd && typeof (cmd as any).action === "string") {
        commands.push(cmd);
      }
    }
  }
...
export function executeCommands(commands: ToolCommand[]): ToolResult[] {
...
export function buildToolSummary(commands: ToolCommand[]): string {
  if (!commands?.length) return "";

  const lines = commands.map((cmd) => {
    switch (cmd.action) {
      case "isolate": {
        const list = (cmd.categories || []).join(", ");
...
export function runSceneCommand(cmd: ToolCommand): ToolResult | undefined {
  const api = (window as any).__SCENE_API__;
  if (!api || !cmd || typeof (cmd as any).action !== "string") return;
  ...
  switch (cmd.action) {
    ...
    default:
      return { action: (cmd as any).action, status: "ignored" };
  }
}
